...
<?php } ?>
  <div class="row" style="padding-bottom: 10px;">
    ...
    <div class="col-xs-12 col-md-1">
      Wajib&nbsp;<input type="checkbox" name="opsi_wajib" value="ya">
    </div>
...
<?php    }elseif ($value->jenis_opsi != 'teks') {
            echo  anchor($my['is'].'/buat_opsi_acara/'.$value->opsi_id,'Buat opsi',array('class'=>'btn btn-primary'));
         }
    }
?>
                </td>
              ...
              <?php  if (!is_null($value->opsi_id)) { 
                            if ($opsi_pertanyaan  = get_induk_opsi($value->opsi_id)) {
                                ...
                                foreach ($opsi_pertanyaan as $key_opsi => $value_opsi) { 
                                    $icon = $icon_jenis[$value->jenis_opsi];
                                    if (!is_null($value_opsi->opsi_jawaban)) {
                                      if ($value_opsi->opsi_jawaban == 0 && $value->jenis_opsi == 'check') {
                                        $icon ='fa fa-square-o';
                                      }else if ($value_opsi->opsi_jawaban == 1 && $value->jenis_opsi == 'radio') {
                                        $icon ='fa fa-dot-circle-o';
                                      }
                                    }
                                    $collapse .='<i class="'.$icon.'"></i>&nbsp;';
